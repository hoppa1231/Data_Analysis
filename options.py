...
DRAWSTYLES = {
    'default': 'Обычный',
    'steps-pre': 'Ступенчатый (перед)', 'steps': 'Ступенчатый (перед)',
    'steps-mid': 'Ступенчатый (центр)',
    'steps-post': 'Ступенчатый (после)'}
...
def figure_edit(axes, parent=None):
    ...
    general = [
        ('Заголовок', axes.get_title()),
        sep,
        *chain.from_iterable([
            (
                (None, f"<b>{name.title()}-Ось</b>"),
                ('Минимум', axis_limits[name][0]),
                ('Максимум', axis_limits[name][1]),
                ('Ярлык', axis.get_label().get_text()),
                ('Шкала', [axis.get_scale(),
                           'linear', 'log', 'symlog', 'logit']),
                sep,
            )
            for name, axis in axis_map.items()
        ]),
        ('(Повторно) Создать автоматическую легенду', False),
    ]
    ...
    for label, line in labeled_lines:
        ...
        curvedata = [
            ...
            sep,
            (None, '<b>Линия</b>'),
            ('Стиль линии', prepare_data(LINESTYLES, line.get_linestyle())),
            ('Стиль рисования', prepare_data(DRAWSTYLES, line.get_drawstyle())),
            ('Толщина', line.get_linewidth()),
            ('Цвет (RGBA)', color),
            sep,
            (None, '<b>Отметки</b>'),
            ('Стиль', prepare_data(MARKERS, line.get_marker())),
            ('Размер', line.get_markersize()),
            ('Основной цвет (RGBA)', fc),
            ('Цвет края (RGBA)', ec)]
    ...
    datalist = [(general, "Оси", "")]
    if curves:
        datalist.append((curves, "Кривая", ""))
    if mappables:
    ...
    _formlayout.fedit(
        datalist, title="Настройки графа", parent=parent,
        icon=QtGui.QIcon(
